...
struct X_KTHREAD {
  ...
  xe::be<uint32_t> context_ptr;       // 0x164
...
class XThread : public XObject, public cpu::Thread {
 public:
  ...
  virtual void SetGuestContext(uint32_t context_ptr, uint32_t code_ptr);
 ...
 protected:
  ...
  bool is_fiber_ = false;
  uint32_t context_ptr_ = 0;
  std::unordered_map<uint32_t, fiber_data> context_fibers_;
